...
#include <rp3d/mathematics/mathematics.h>
...
namespace std {
  ...
  template <> struct hash<rp3d::DebugRenderer::DebugItem> {

    size_t operator()(const rp3d::DebugRenderer::DebugItem& debugItem) const {

        return std::hash<rp3d::uint32>{}(static_cast<unsigned int>(debugItem));
    }
  };
}
